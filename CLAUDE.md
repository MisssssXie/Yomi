...
Yomi（月讀）是一個 macOS 專用的本地 web app：用 QuickTime 錄音 → 透過 `nlm` CLI 上傳到 NotebookLM → 拿回逐字稿與摘要。
...
- macOS 需允許 QuickTime Player 的麥克風與「自動化」權限
...
**錄音**：透過 `osascript` 控制 QuickTime Player，存到 `recordings/<id>.<ext>`。同時只能錄一份。
